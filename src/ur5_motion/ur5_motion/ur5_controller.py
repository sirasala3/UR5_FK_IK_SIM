...
class UR5JazzyController(Node):
    ...
    def run_pick_and_place(self, pick_pose, place_pose, approach_dist=0.1, retreat_dist=0.1):
        """
        Executes a complete pick and place sequence.

        Args:
            pick_pose (Pose): The final target pose for grasping the object.
            place_pose (Pose): The final target pose for placing the object.
            approach_dist (float): The distance to approach the object from.
            retreat_dist (float): The distance to retreat from the object after grasping/placing.
        """
        ...
        approach_pose.position.z = pick_pose.position.z + approach_dist
        approach_pose.orientation = pick_pose.orientation
        self.execute_waypoint_path([approach_pose], duration_per_segment=0.5, avoid_obstacles=True)
        self.get_logger().info('Approached pick pose.')

        # 2. Pick the object
        # NOTE: You need to implement gripper logic here
        self.execute_waypoint_path([pick_pose], duration_per_segment=0.5, avoid_obstacles=True)
        # close_gripper() # Uncomment when implemented
        time.sleep(1) # Simulates grasp action
        self.get_logger().info('Object picked.')

        # 3. Retreat from pick position
        retreat_pose = Pose()
        retreat_pose.position.x = pick_pose.position.x
        retreat_pose.position.y = pick_pose.position.y
        retreat_pose.position.z = pick_pose.position.z + retreat_dist
        retreat_pose.orientation = pick_pose.orientation
        self.execute_waypoint_path([retreat_pose], duration_per_segment=0.5, avoid_obstacles=True)
        self.get_logger().info('Retreated from pick pose.')

        # 4. Move to place position
        approach_place_pose = Pose()
        approach_place_pose.position.x = place_pose.position.x
        approach_place_pose.position.y = place_pose.position.y
        approach_place_pose.position.z = place_pose.position.z + approach_dist
        approach_place_pose.orientation = place_pose.orientation
        self.execute_waypoint_path([approach_place_pose], duration_per_segment=0.5, avoid_obstacles=True)
        self.get_logger().info('Moved to place approach pose.')
        
        # 5. Place the object
        self.execute_waypoint_path([place_pose], duration_per_segment=0.5, avoid_obstacles=True)
        # open_gripper() # Uncomment when implemented
        time.sleep(1) # Simulates release action
        self.get_logger().info('Object placed.')

        # 6. Retreat from place position
        retreat_pose = Pose()
        retreat_pose.position.x = place_pose.position.x
        retreat_pose.position.y = place_pose.position.y +0.15
        retreat_pose.position.z = place_pose.position.z + retreat_dist
    ...
    def pick_place_demo(self):
        """
        A demonstration function that uses the new, generic pick and place method.
        """
        self.get_logger().info('Running pick and place demonstration.')

        # Define the pick and place poses directly
        pick_pose = Pose()
        pick_pose.position.x, pick_pose.position.y, pick_pose.position.z = [0.4, 0.3, 0.3]
        qx, qy, qz, qw = quaternion_from_euler(0, 1.57, 0)
        pick_pose.orientation.x, pick_pose.orientation.y, pick_pose.orientation.z, pick_pose.orientation.w = qx, qy, qz, qw

        place_pose = Pose()
        place_pose.position.x, place_pose.position.y, place_pose.position.z = [0.4, -0.3, 0.3]
        qx, qy, qz, qw = quaternion_from_euler(0, 1.57, 0)
        place_pose.orientation.x, place_pose.orientation.y, place_pose.orientation.z, place_pose.orientation.w = qx, qy, qz, qw
        
        self.run_pick_and_place(pick_pose, place_pose)
        self.get_logger().info('Pick and place demonstration finished.')
